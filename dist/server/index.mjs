import _, { fromPairs, toPairs, pick, head } from "lodash";
import Joi from "joi";
import qs from "qs";
...
import fromPairs$1 from "lodash/fromPairs";
...
Object.keys(errorMessages).forEach(
  (k) => errorMessages[k] = _.template(errorMessages[k], {
    interpolate: /\{\s*(\S+)\s*\}/g
  })
);
...
function linkMediaAttributes(schema, fileEntry, { fileIdToDbId }) {
  for (const [attributeName, attribute] of Object.entries(schema.attributes)) {
    ...
    if (isMediaAttribute(attribute)) {
      attributeValue = castArray(attributeValue).map((id) => fileIdToDbId.getMapping("plugin::upload.file", id)).filter(Boolean);
...
const updateOrCreateCollectionTypeEntry = async (user, slug, fileId, fileEntry, { idField, importStage, fileIdToDbId }) => {
  ...
  if (!schema.pluginOptions?.i18n?.localized) {
    ...
    if (isEmpty$1(omit(fileEntry, ["id"]))) {
      return null;
    }
    if (isDatumInDefaultLocale) {
      if (!dbEntryDefaultLocaleId) {
